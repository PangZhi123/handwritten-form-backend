...
- 原始模板只读，结果始终写入副本。
- 固定文字默认保留，不被模型结果覆盖。
- 同时识别印刷与手写内容；模型无需返回行列数，输出结构始终取自DOCX模板。
- 未返回的表格或单元格保留模板原文，同一表格的多个片段按坐标合并；无识别内容时填充数为0，需要核对识别结果。
- 未知表格编号、无效或被合并覆盖的坐标、同一格返回冲突文字时返回 `HF0204`，不猜测或错位填充。
